...
- 顶部及用户追踪统一使用六类筛选：全部用户、退费用户、未退费用户、续费用户、未退费未续费用户、潜在流失风险用户；
...
- **用户追踪**：以自然月为单元，四周每周 2 节并加 1 节月度课，共 9 节；点击用户名后原表切换为该学生的课时多维明细。参课课时可继续展开单次会话回放（含 `SES-8842` 行为还原），并沿「连续表现 → 异常信号 → 情绪识别 → 断点优化」定位产品机会。
