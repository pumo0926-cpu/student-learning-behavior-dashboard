...
function animationBehavior(user, lessonIndex, isBreak, totalSec) {
  const seed = Number(user.id.slice(-2)) + lessonIndex * 7;
  ...
  let wall = 12, pos = 0, play = 0, maxPos = 0;   // 12s = 进入动画的时刻
  const marks = [];
  for (const m of raw) {
    if (m.at < pos) continue;                       // 已被拖过去的位置不再触发
    wall += m.at - pos; play += m.at - pos; pos = m.at; maxPos = Math.max(maxPos, pos);
    marks.push({ ...m, atSession: Math.round(wall), atVideo: Math.round(pos), toVideo: m.to != null ? Math.round(m.to) : null });
    if (m.kind === "pause") wall += m.hold; else { pos = m.to; maxPos = Math.max(maxPos, pos); }
  }
  const endPos = isBreak ? Math.min(videoSec, pos + videoSec * .18) : videoSec;
  wall += Math.max(0, endPos - pos); play += Math.max(0, endPos - pos);
...
function lessonSessionReplay(user, lessonIndex, record) {
  ...
  const anim = animationBehavior(user, lessonIndex, isBreak, totalSec);
  ...
  anim.marks.forEach(m => events.push(m.kind === "pause"
    ? [mmss(m.atSession), `暂停 · 视频 ${mmss(m.atVideo)}`, `pause · 停留 ${m.hold} 秒`, "warn"]
  ...
  if (isBreak) {
    // 静默必须落在最后一题之后。固定取「总时长 − 90s」时，若题目间隔本就小于 90s，
    // 会被插到最后一题前面，时间轴出现倒挂。这里锚在最后一题之后，并按实际间隔报时长。
    const idleAt = Math.max(learnEnd + slot * qs.length + 8, totalSec - 90);
    events.push([mmss(idleAt), "静默无操作", `idle · ${Math.round(totalSec - idleAt)} 秒未响应`, "warn"]);
  }
  ...
  const timelineFlags = anim.marks.length + instant.length + repeat.length + slow.length + (isBreak ? 1 : 0);
  const chain = [];
